...
void decompileAST_LEXP_START(AST *node, FILE *file)
{
    decompile(node->son[0], file);
    fprintf(file, " ( ");
    decompile(node->son[1], file);
...
void decompile(AST *node, FILE *file)
{
    ...
    switch (node->type)
    {
        ...
        case AST_LEXP_START:                decompileAST_LEXP_START(node, file);                break;
        ...
        case AST_FUNC:                      decompileAST_FUNC(node, file);                      break;
        case AST_PARA_START:                decompileAST_PARA_START(node, file);                break;
        case AST_PARA_NEXT:                 decompileAST_PARA_NEXT(node, file);                 break;
        case AST_PARA:                      decompileAST_PARA(node, file);                      break;

        case AST_LDEC:                      decompileAST_LDEC(node, file);                      break;
        case AST_VARI:                      decompileAST_VARI(node, file);                      break;
        case AST_VARF:                      decompileAST_VARF(node, file);                      break;

        case AST_VECT:                      decompileAST_VECT(node, file);                      break;
