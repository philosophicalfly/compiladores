...
programa: 
    listOfDeclarations                                                          { $$ = $1; ast = $$; astPrint($1, 0);}
    ;
...
varDeclaration:
    KW_INT TK_IDENTIFIER ':' singleLiteral ';'                                  { $$ = branchUp(AST_VARI, symbolize($1), symbolize($2), $4, 0, 0); }
    | KW_CHAR TK_IDENTIFIER ':' singleLiteral ';'                               { $$ = branchUp(AST_VARI, symbolize($1), symbolize($2), $4, 0, 0); }
    | KW_FLOAT TK_IDENTIFIER ':' LIT_INTEGER '/' LIT_INTEGER ';'                { $$ = branchUp(AST_VARF, symbolize($1), symbolize($2), symbolize($4), symbolize($6), 0); }
    ;

vectorDeclaration:
    KW_INT TK_IDENTIFIER '[' singleLiteral ']' vectorInitializationList         { $$ = branchUp(AST_VECT, symbolize($1), symbolize($2), $4, $6, 0); }
    | KW_CHAR TK_IDENTIFIER '[' singleLiteral ']' vectorInitializationList      { $$ = branchUp(AST_VECT, symbolize($1), symbolize($2), $4, $6, 0); }
    | KW_FLOAT TK_IDENTIFIER '[' singleLiteral ']' vectorInitializationList     { $$ = branchUp(AST_VECT, symbolize($1), symbolize($2), $4, $6, 0); }
    ;
...
functionDeclaration:
    KW_CHAR TK_IDENTIFIER '(' parameterList ')' command                         { $$ = branchUp(AST_FUNC, symbolize($1), symbolize($2), $4, $6, 0); }
    | KW_INT TK_IDENTIFIER '(' parameterList ')' command                        { $$ = branchUp(AST_FUNC, symbolize($1), symbolize($2), $4, $6, 0); }
    | KW_FLOAT TK_IDENTIFIER '(' parameterList ')' command                      { $$ = branchUp(AST_FUNC, symbolize($1), symbolize($2), $4, $6, 0); }
    ;
...
parameter:
    KW_CHAR     TK_IDENTIFIER                                                   { $$ = branchUp(AST_PARA, symbolize($1), symbolize($2), 0, 0, 0); }
    | KW_INT    TK_IDENTIFIER                                                   { $$ = branchUp(AST_PARA, symbolize($1), symbolize($2), 0, 0, 0); }
    | KW_FLOAT  TK_IDENTIFIER                                                   { $$ = branchUp(AST_PARA, symbolize($1), symbolize($2), 0, 0, 0); }
    ; 
...
expression: 
    ...
    | TK_IDENTIFIER '(' listOfExpressions ')'                                   { $$ = branchUp(AST_LEXP_START, symbolize($1), $3, 0, 0, 0); }
